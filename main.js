...
// Datos de stock cargados desde Supabase
let stockData = {};

// Lista de productos en el carrito
let cart = [];

// Oculta el age gate y muestra el sitio con animación
function enterSite() {
  ...
  setTimeout(() => {
    ...
    // Muestra el botón flotante de WhatsApp
...
// Muestra la pantalla de rechazo si el usuario es menor de edad
function rejectUser() {
  document.getElementById('age-gate').style.display = 'none';
  document.getElementById('rejected').classList.add('show');
}

// Abre el catálogo con transición de opacidad
...
// Vuelve a la pantalla principal desde el catálogo
function backToHome() {
  const site = document.getElementById('site');
...
// Cambia la pestaña activa de marca en el catálogo
...
// Cierra la sección "Nosotros" y vuelve al inicio
...
// Cierra la sección "Contacto" y vuelve al inicio
...
// Abre el carrito y oculta el botón flotante de WhatsApp
function openCart() {
...
// Cierra el carrito y muestra el botón flotante de WhatsApp
function closeCart() {
...
// Agrega un producto al carrito solo si tiene stock disponible
...
// Cambia la cantidad de un producto en el carrito (delta: +1 o -1)
...
// Actualiza la vista del carrito y el enlace de WhatsApp con el pedido
function updateCart() {
  ...
  let total=0;
  // Genera el HTML de cada ítem del carrito
...
// Actualiza el contador de ítems en todos los botones de carrito
function updateCartCount() {
  const total = cart.reduce((sum,i) => sum+i.qty, 0);
  document.querySelectorAll('.cart-count').forEach(el => el.textContent=total);
}

// Carga el stock de productos desde Supabase y renderiza el catálogo
async function loadStock() {
  try {
    ...
    // Guarda cada producto en stockData con clave "marca|sabor"
    data.forEach(item => { stockData[item.marca+'|'+item.sabor] = item; });
    renderCatalog(data);
  } catch(e) { console.error('Stock error:', e); }
}

// Genera y muestra las tarjetas de productos en el catálogo
function renderCatalog(data) {
  const brands = {};
  // Agrupa los productos activos por marca
  ...
  function buildCard(item) {
    // Busca la imagen local; si no hay, muestra un ícono de caja
    ...
    // Badge de stock: sin stock / últimas unidades / disponibles
  ...
  // Panel "Todos": muestra todas las marcas juntas
  ...
  // Paneles individuales por marca
...
// Actualiza los badges de stock en todas las tarjetas del catálogo
function updateAllStockBadges() {
  document.querySelectorAll('.product-card').forEach(card => {
    ...
    // Extrae la marca del atributo onclick del botón
...
// Abre el panel de administración y resetea el formulario de login
function openAdmin() {
...
// Verifica la contraseña del admin y muestra la tabla si es correcta
function checkAdmin() {
...
// Genera la tabla de stock en el panel de administración
function renderAdminTable() {
  const tbody = document.getElementById('adminTableBody');
  tbody.innerHTML='';
  // Ordena los productos por marca y crea una fila por cada uno
...
// Cambia el stock de un producto en memoria (sin guardar aún en la base)
...
// Guarda el stock actualizado en Supabase
...
// Cantidad de estrellas seleccionadas en el formulario de reseñas
let estrellasSeleccionadas = 5;

// Actualiza las estrellas activas según la selección del usuario
...
// Carga las reseñas aprobadas desde Supabase y las muestra
async function loadResenas() {
  try {
    ...
    // Genera una tarjeta por cada reseña
...
// Envía una nueva reseña a Supabase (queda pendiente de aprobación)
async function enviarResena() {
  ...
  try {
    ...
    // activo: false porque la reseña espera aprobación del admin
...
// Abre la sección de reseñas y las carga
...
// Cierra la sección de reseñas y vuelve al inicio
...
// Carga todas las reseñas (incluyendo pendientes) para el panel admin
async function loadResenasAdmin() {
  try {
    ...
    // Genera una tarjeta por reseña con botones de aprobar/eliminar
...
// Aprueba una reseña (la hace visible para todos los usuarios)
